...
public:
    ...
    void ComputingStockRt(AbnormalMap& abnormalReturn, StockMap& StockList, const vector<double> &Rmt);
    vector<double> ComputingAAR(AbnormalMap& abnormalReturn);
    vector<double> ComputingCAAR(vector<double>& calculation);
    vector<vector<vector<double>>> LastComputing(GroupOfName& grouplist, vector<double>& Rmt, AbnormalMap& abnormalReturn);
